...
public class ClickGuiInputHandler {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button, Window window) {
        ...
        if (button == 0 && MathUtil.isHovered(mouseX, mouseY, searchX, searchY,
        ...
        for (int i = 0; i < categories.length; i++) {
                ...
                if (button == 0) {
                    ...
                    for (Theme themeItem : Quilt.getInstance().getThemeManager().getThemes()) {
                        if (MathUtil.isHovered(mouseX, mouseY, panelX + 4, themeY, ClickGuiLayout.WIDTH - 8, 18)) {
                            Quilt.getInstance().getThemeManager().setCurrentTheme(themeItem);
                            return true;
                        }
                        themeY += 20.0f;
                    }
            ...
            for (Module module : state.getModules(category)) {
                ...
                if (MathUtil.isHovered(mouseX, mouseY,
                        ...
                        ClickGuiLayout.MODULE_INNER_WIDTH, ClickGuiLayout.MODULE_HEADER_HEIGHT)) {
                    if (button == 0) {
                        module.toggle();
                        return true;
                    }
                ...
                if (state.isModuleOpen(module) && openProgress > 0.1f) {
                    if (handleSettingClick(mouseX, mouseY, button,
                            panelX + ClickGuiLayout.MODULE_PADDING, moduleY, module.getSettings())) {
                        return true;
                    }
                }
        ...
        for (int i = 0; i < visibleSettings.size(); i++) {
            ...
            if (setting instanceof BooleanSetting booleanSetting) {
                ...
                if (button == 0 && MathUtil.isHovered(mouseX, mouseY,
                        ...
                        ClickGuiLayout.SLIDER_WIDTH, 6)) {
                    floatSetting.setCurrent(state.getSliderValue(floatSetting,
                            moduleX + ClickGuiLayout.SETTING_LEFT, mouseX));
