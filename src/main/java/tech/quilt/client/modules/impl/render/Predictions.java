   ...
   public void onWorldRender(EventRender3D e) {
      ...
      this.getProjectiles().forEach((entity) -> {
         ...
         for(int i = 0; i < 300; ++i) {
            ...
            Render3DUtil.drawLine(prevPos, pos, Quilt.getInstance().getThemeManager().getClientColor(i).mulAlpha(MathHelper.clamp((float)i / 25.0F, 0.0F, 1.0F)).getRGB(), 2.0F, false);
