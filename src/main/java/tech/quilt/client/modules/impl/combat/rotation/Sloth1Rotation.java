...
public class Sloth1Rotation extends RotationBase {
    ...
    public void update(LivingEntity target, Rotation targetAngle, boolean elytraVisual) {
        ...
        float baseYawSpeed = (float) (Math.random() * 6.0 + 10.0) * speedMultiplier / (isElytra ? 2.5F : 1.0F);
        float basePitchSpeed = (float) (Math.random() * 3.0 + 5.0) * speedMultiplier / (isElytra ? 2.5F : 1.0F);
        ...
        if (hasTrace && Math.abs(clampedYaw) < 0.5F) clampedYaw = 0.0F;
        if (hasTrace && Math.abs(clampedPitch) < 0.3F) clampedPitch = 0.0F;
