package tech.quilt.client.hud.elements.draggable;
...
public abstract class DraggableHudElement implements IMinecraft {
   ...
   protected void renderYLine(CustomDrawContext ctx, DraggableHudElement.SheetCode nearest) {
      if (nearest.pos == -1.0F) {
         ...
         ctx.drawRoundedRect(0.0F, nearest.pos, (float)ctx.getScaledWindowWidth(), 1.0F, BorderRadius.ZERO, Quilt.getInstance().getThemeManager().getCurrentTheme().getColor().mulAlpha(1.0F));
