package tech.quilt.base.discord;
...
import tech.quilt.base.discord.utils.DiscordRichPresence;
import tech.quilt.base.discord.utils.RPCButton;
import tech.quilt.utility.render.display.BufferUtil;
...
public class DiscordManager {
   ...
   private void initRPC() {
      try {
         DiscordEventHandlers handlers = (new DiscordEventHandlers.Builder()).ready((user) -> {
            Quilt.getInstance().getDiscordManager().setInfo(new DiscordManager.DiscordInfo(user.username, "https://cdn.discordapp.com/avatars/" + user.userId + "/" + user.avatar + ".png", user.userId));
            Quilt.getInstance().getDiscordManager().setInfo(new DiscordManager.DiscordInfo(user.username, "https://cdn.discordapp.com/avatars/" + user.userId + "/" + user.avatar + ".png", user.userId));
            DiscordRichPresence richPresence = (new DiscordRichPresence.Builder()).setStartTimestamp(System.currentTimeMillis() / 1000L).setDetails("USER » vorkis").setState("UID » 1").setLargeImage("logo").setButtons(RPCButton.create("Buy Client", "https://quiltclient.fun"), RPCButton.create("Discord", "https://discord.gg/hYgEF3gYzX")).build();
   ...
   private class DiscordDaemonThread extends Thread {
      public void run() {
         ...
         try {
            while(Quilt.getInstance().getDiscordManager().isRunning()) {
