package tech.quilt.utility.game.player;
...
public final class PlayerInventoryComponent implements IMinecraft {
   ...
   public static void addTask(Runnable task) {
      if (MovingUtil.hasPlayerMovement()) {
         Quilt.getInstance().getScriptManager().addTask(script);
         String var1 = Quilt.getInstance().getServerHandler().getServer();
         byte var2 = -1;
   ...
   public static void updateMoveKeys() {
      if (!Quilt.getInstance().getMenuScreen().search) {
         moveKeys.forEach((keyBinding) -> {
