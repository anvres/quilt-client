...
public class Sloth2Rotation extends RotationBase {
    ...
    public void update(LivingEntity target, Rotation targetAngle, boolean elytraVisual) {
        ...
        float baseYawSpeed = (float) (Math.random() * 7.0 + 12.0) * speedMultiplier / (isElytra ? 2.0F : 1.0F);
        float basePitchSpeed = (float) (Math.random() * 4.0 + 6.0) * speedMultiplier / (isElytra ? 2.0F : 1.0F);
        ...
        if (hasTrace && Math.abs(clampedPitch) < 0.2F) clampedPitch = 0.0F;
