...
public class ClickGuiRenderer {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, Window window, float animationProgress) {
        ...
        int colorTheme = Quilt.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB();
    ...
    private void renderSearchField(DrawContext context, float alphaMul, int categoryCount) {
        ...
        DrawUtil.drawBlur(customDrawContext.getMatrices(), searchX, searchY,
                ...
                15.0f, BorderRadius.all(4.0f), ColorRGBA.WHITE.withAlpha((int) (255 * alphaMul)));
        DrawUtil.drawRoundedRect(customDrawContext.getMatrices(), searchX, searchY,
                ClickGuiLayout.SEARCH_WIDTH, ClickGuiLayout.SEARCH_HEIGHT,
                BorderRadius.all(4.0f), new ColorRGBA(0, 0, 0, (int) (125 * alphaMul)));
    ...
    private void renderCategoryPanel(DrawContext context, int mouseX, int mouseY, float panelX,
        ...
        DrawUtil.drawRoundedRect(customDrawContext.getMatrices(), panelX, panelY,
                ClickGuiLayout.WIDTH, ClickGuiLayout.HEIGHT, BorderRadius.all(rounding), bodyColor);
        DrawUtil.drawRoundedRect(customDrawContext.getMatrices(), panelX, panelY,
                ClickGuiLayout.WIDTH, headerHeight, new BorderRadius(rounding, rounding, 0, 0), headerColor);

        String icon = category.getIcon();
        float iconW = Fonts.ICONS.getWidth(icon, 9.0f);
        ...
        customDrawContext.drawText(Fonts.ICONS.getFont(9.0f), icon,
                headerStartX + iconOffsetX, panelY + 6.0f, themeColor.withAlpha((int) (255 * alphaMul)));
        customDrawContext.drawText(Fonts.REGULAR.getFont(10.0f), category.getName(),
                headerStartX + iconW + gap, panelY + 5.0F, themeColor.withAlpha((int) (255 * alphaMul)));
        ...
        DrawUtil.drawRoundedRect(customDrawContext.getMatrices(), panelX, contentY,
                ...
                new BorderRadius(0, 0, rounding, rounding), ColorRGBA.WHITE);
        StencilUtil.read(1);

        if (category == Category.THEMES) {
            float themeY = contentY + 2.0F + state.getScroll(category);
            for (Theme themeItem : Quilt.getInstance().getThemeManager().getThemes()) {
    ...
    private void renderThemeItem(DrawContext context, int mouseX, int mouseY, float panelX,
        ...
        CustomDrawContext customDrawContext = CustomDrawContext.of(context);
        Theme currentTheme = Quilt.getInstance().getThemeManager().getCurrentTheme();
        ...
        ColorRGBA bg = isSelected
                ? themeItem.getColor().withAlpha((int) (80 * alphaMul))
                : (isHovered
                ? new ColorRGBA(255, 255, 255, (int) (20 * alphaMul))
                : new ColorRGBA(255, 255, 255, (int) (10 * alphaMul)));
    ...
    private void renderModule(DrawContext context, int mouseX, int mouseY, float panelX,
                              float moduleY, Module module, float openProgress,
                              float moduleHeight, int colorTheme, float alphaMul) {
        CustomDrawContext customDrawContext = CustomDrawContext.of(context);
        Theme theme = Quilt.getInstance().getThemeManager().getCurrentTheme();
        ColorRGBA themeColor = theme.getColor();

        ColorRGBA moduleBg = module.isEnabled()
                ? themeColor.withAlpha((int) (80 * alphaMul))
                : new ColorRGBA(255, 255, 255, (int) (10 * alphaMul));

        DrawUtil.drawRoundedRect(customDrawContext.getMatrices(),
                panelX + ClickGuiLayout.MODULE_PADDING, moduleY,
                ClickGuiLayout.MODULE_INNER_WIDTH, moduleHeight,
                BorderRadius.all(2.0f), moduleBg);
        ...
        if (dotProgress > 0.01f) {
            ...
            DrawUtil.drawRoundedRect(customDrawContext.getMatrices(), dotX, dotY, dotSize, dotSize,
                    BorderRadius.all(dotSize / 2.0f), themeColor.withAlpha((int) (255 * alphaMul * dotProgress)));
        ...
        customDrawContext.drawText(Fonts.REGULAR.getFont(7.5f), moduleName,
                panelX + ClickGuiLayout.SETTING_LEFT + textOffsetX + 2.0f,
                moduleY + 7.5f, nameColor.withAlpha((int) (255 * alphaMul)));
    ...
    private void renderModuleDots(DrawContext context, float panelX, float moduleY,
                                  Module module, boolean enabled, float alphaMul) {
        CustomDrawContext customDrawContext = CustomDrawContext.of(context);
        float dotsX = panelX + ClickGuiLayout.WIDTH - 12f;
        ColorRGBA dotsColor = enabled
                ? ColorRGBA.WHITE.withAlpha((int) (220 * alphaMul))
                : ColorRGBA.WHITE.withAlpha((int) (150 * alphaMul));
